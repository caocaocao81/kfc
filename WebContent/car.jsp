...
<!--    javascript代码-->
    <script>
        ...
        $(function () {//这是jquery的入门函数
            ...
            //筛选元素
            //$("input")  所有的input标签
            //$("input[name='mid']") input标签中  name属性值是mid的元素
            //$("input[name='mid']").click(function(){})  为选中的元素绑定click事件，单击是执行function的代码
            //复选框单击时可能是选中也可能是取消
...
<body>
<form action="meal/pay" method="post" onsubmit="return checkForm()">
<table>
    <tr>
        <th>
    ...
    <!-- 遍历购物车 -->
    <c:forEach items="${car}" var="meal">
    <tr>
        <td>
            <input type="checkbox" name="mid" value="${meal.id }">
        </td>
        <td>${meal.mname }</td>
        <td class="singlePrice">${meal.price }</td>
    </tr>
    </c:forEach>
    <tr>
